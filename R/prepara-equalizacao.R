...
#' @param var_alunos_avaliada coluna numerica com o numero de alunos por ente
#'
...
prepara_equalizacao <- function(dados, var_vaa = vaa, var_alunos_avaliada = alunos_estado){
  dados %>%
    dplyr::arrange({{var_vaa}}) %>%
    dplyr::mutate(
      cumulativo_alunos = cumsum({{var_alunos_avaliada}}),
      necessario_equalizacao = ({{var_vaa}} * cumulativo_alunos) - cumsum({{var_vaa}} * {{var_alunos_avaliada}})
    )
  }
