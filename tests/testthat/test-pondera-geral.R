test_that("pondera_geral funciona", {
  ...
  expect_equal(pondera_geral(alunos_teste, ponderador_alunos, socioeco_teste, financas_teste, max_financas = 2, min_financas = 0.2) %>% dplyr::pull(financas), c(2, 0.2, 0.2, 0.2, 2))
})
...
test_that("pondera_geral gera data.frame", {
  ...
  expect_output(pondera_geral(alunos_teste, ponderador_alunos, socioeco_teste, financas_teste, min_social = 0.5, max_social = 2, min_financas = 3, max_financas = 4) %>% str, "data\\.frame")
})
